<script lang="ts">
  ...
  async function connectWalletTestnet() {
  ...
  var isMetamaskConnected = async () => {
    const tokenContractAddress = "0xe5a50c2b0a2538838c9ABD8febC17Ee1CdFA3120";
    if (window.ethereum) {
      ...
      if (accounts.length !== 0) {
        ...
        let balanceStr = String(_balance);

        // Ethereum balance
        testEth.set(balanceStr);
        account.set(accounts[0]);

        let contract = new ethers.Contract(tokenContractAddress, abi, provider);
        // ERC20 balance
        let tokenBalance = await contract.balanceOf(accounts[0]);
        let tokenBalanceStr = String(tokenBalance);

        // Set the ERC20 balance
        mirrorChips.set(tokenBalanceStr);
      } else {
  ...
  //continiously verify metamask information
  let clear: any;
  $: {
    clearInterval(clear);
    clear = setInterval(isMetamaskConnected, 1000);
  }
  //style metamask button
  $: activeStyle = $account
